 ...
 * @description Full-fidelity G-code parser for the FDM digital shadow.
 *
 * Parses a .gcode file (string, File object, or URL) into a move list that
 * `PrintingMotion.loadMoves()` can execute. All supported commands produce
 * either motion entries consumed by the axis/filament system, or state entries
 * consumed by the PrintingMotion executor for realistic printing behaviour.
 *
 * Supported commands
 * ──────────────────
 *  Motion
...
export class GCodeLoader {
  ...
  parse(text) {
    ...
    for (const rawLine of lines) {
      ...
      if (cmd === 'G92') {
        const move = { cmd: 'G92' };
        // G92 with no parameters resets E to 0 (Marlin convention)
